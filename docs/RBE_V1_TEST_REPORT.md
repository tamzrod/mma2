...
Historical: the old in-process sink could split a rule name with CR/LF into
multiple line-protocol points. That sink is gone. `BuildRBERules` still rejects
rule names containing line breaks.
...
3. Influx is not part of MMA; history is an external subscriber concern.
