...
MMA2 is the appliance. RBE subscribers are generic TCP clients. A plant controller is one use case.
...
| 2 | Fix `TCPPublisher` lock | Real deadlock on overflow/Close. Overflow is the designed slow-subscriber path. | Small | Close and overflow no longer hold `mu` while closing queues. Contention test passes. |
...
| 8 | Neutral example YAML + docs pass | **Language pass started 2026-09-18.** Remaining: finish README / `docs/02_ARCHITECTURE.md` truncation. Example already shows HR + IR. | Small–medium | Example shows HR via Modbus **and** IR via Raw Ingest. README / `docs/02_ARCHITECTURE.md` no longer end at `## Archi`. |
...
| 11 | Subscriber p95/p99 | Valuable only after 1–7. Measuring an unlocked contract wastes the run. | Large | Measured trigger→Modbus-read distribution. No invented numbers in docs. |
